      ...
      <div className="flex">
        ...
        <div className="flex-1 max-w-[360px]">
          ...
          <div className="flex mb-1">
            ...
            {levels.map((level) => (
              <div key={level} className="flex-1 text-center text-sm font-medium text-muted-foreground px-1">
                {levelLabels[level]}
              </div>
            ))}
          </div>
          ...
          {[...levels].reverse().map((prob, reversedProbIndex) => {
            ...
            return (
              <div key={prob} className="flex items-center mb-1">
                {/* Rad-label */}
                <div className="w-16 text-sm font-medium text-muted-foreground text-right pr-2">
                  {levelLabels[prob]}
                </div>
                {/* Celler */}
                {levels.map((cons, consIndex) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "flex-1 aspect-square m-0.5 rounded-md flex items-center justify-center text-base font-bold transition-all cursor-pointer",
                        getRiskColor(score),
