...
export function AddRiskDialog({
  ...
  const calculateRiskScore = (probability: string, consequence: string): number => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const riskScore = calculateRiskScore(validatedData.probability, validatedData.consequence);
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          })} placeholder="Beskriv risikoen kort..." maxLength={200} />
          ...
          })} placeholder="Detaljert beskrivelse av risikoen..." rows={3} maxLength={1000} />
          ...
          })} placeholder="F.eks. Datahåndtering, Sikkerhet, Prosess..." maxLength={100} />
          ...
          <div className="space-y-2">
            ...
            <RiskMatrix probability={formData.probability} consequence={formData.consequence} onSelect={(probability, consequence) => setFormData({
            ...formData,
            probability: probability as RiskFormData["probability"],
            consequence: consequence as RiskFormData["consequence"]
          })} />
            {(errors.probability || errors.consequence) && <p className="text-sm text-destructive">
          ...
          <div className="space-y-2">
            ...
            <Select value={formData.treatment} onValueChange={value => setFormData({
            ...formData,
          ...
          <div className="space-y-2">
            ...
            <Textarea id="mitigation_plan" value={formData.mitigation_plan} onChange={e => setFormData({
            ...formData,
            mitigation_plan: e.target.value
          })} placeholder="Beskriv konkrete tiltak for å håndtere risikoen..." rows={4} maxLength={2000} />
            {errors.mitigation_plan && <p className="text-sm text-destructive">
                {errors.mitigation_plan}
              </p>}
          </div>

          {/* Viser beregnet risikoscore hvis begge verdier er valgt */}
          {formData.probability && formData.consequence && <div className="p-4 bg-muted rounded-md">
              <p className="text-sm font-medium">
                Beregnet risikoscore:{" "}
                {calculateRiskScore(formData.probability, formData.consequence)}{" "}
                (
                {getRiskLabel(calculateRiskScore(formData.probability, formData.consequence))}
                )
              </p>
            </div>}
